...
def drawCross(img, center, r, g, b):
    ...
    ctrx = center[0, 0]
    ctry = center[0, 1]
    cv2.line(img, (ctrx - d, ctry - d),
             (ctrx + d, ctry + d), color, t, LINE_AA)
    cv2.line(img, (ctrx + d, ctry - d),
             (ctrx - d, ctry + d), color, t, LINE_AA)
...
def mouseCallback(event, x, y, flags, null):
    ...
    if previous_x > 0:
        heading = np.arctan2(
            np.array([y-previous_y]), np.array([previous_x-x]))
        ...
        distance = np.linalg.norm(
            np.array([[previous_x, previous_y]])-np.array([[x, y]]), axis=1)
...
def update(particles, weights, z, R, landmarks):
    ...
    for i, landmark in enumerate(landmarks):
        ...
        distance = np.power(
            (particles[:, 0] - landmark[0])**2 + (particles[:, 1] - landmark[1])**2, 0.5)
...
landmarks = np.array([[144, 73], [410, 13], [336, 175], [
                     718, 159], [178, 484], [665, 464]])
NL = len(landmarks)
...
while(1):
    ...
    for particle in particles:
        cv2.circle(img, tuple((int(particle[0]), int(
            particle[1]))), 1, (255, 255, 255), -1)
    ...
    cv2.putText(img, "Robot Trajectory(Ground truth)",
                (30, 60), 1, 1.0, (0, 255, 0))
